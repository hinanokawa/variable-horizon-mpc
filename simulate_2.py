...
# ==============================
# (ii) 小さな外乱が常時存在する場合（正弦波）
# ==============================

def disturbance(t):
    # 常時存在する微小外乱（モデル不一致・再エネ微変動を模擬）
    w = 0.05 * np.sin(0.2 * t)
    return w
...
axs[0].plot(t_grid, Xv, '-', color="red", label="Variable MPC")
axs[0].axhline(h_ref, ls=":", color="gray", label="Reference")
axs[0].set_ylabel("Water Level h [m]")
axs[0].set_title("(a) Water Level Response")
axs[0].legend()
axs[0].grid(True)

# (b) Control Input
axs[1].plot(t_grid, Uf, '--', label="Fixed MPC")
axs[1].plot(t_grid, Uv, '-', color="red", label="Variable MPC")
...
# (c) Horizon Switching
axs[2].step(t_grid, Nf, '--', where="post", label="Fixed MPC")
axs[2].step(t_grid, Nv, '-', where="post", color="red", label="Variable MPC")
axs[2].set_ylabel("Prediction Horizon N [steps]")
axs[2].set_title("(c) Horizon Switching Behavior")
axs[2].legend()
axs[2].grid(True)

# (d) Slack Indicator (EMA)
axs[3].plot(t_grid, Sf, '--', label="Fixed MPC")
axs[3].plot(t_grid, Sv, '-', color="red", label="Variable MPC")
axs[3].set_ylabel("EMA of Prediction Error [–]")
axs[3].set_title("(d) Exponential Moving Average of Prediction Error")
axs[3].legend()
axs[3].grid(True)

# (e) MPC Computation Time
axs[4].plot(t_grid, Tf, '--', label="Fixed MPC")
axs[4].plot(t_grid, Tv, '-', color="red", label="Variable MPC")
axs[4].set_ylabel("Computation Time [s]")
axs[4].set_xlabel("Time [s]")
axs[4].set_title("(e) MPC Computation Time per Step")
axs[4].legend()
axs[4].grid(True)
